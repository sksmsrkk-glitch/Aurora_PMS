...
| 운영 스키마 | 51 tables, 34 triggers, 79 validated foreign keys, 51 RLS-enabled tables, 14 migrations |
| 자동 테스트 | production build + 34 tests + 보안 회귀·홈페이지 CMS E2E |
...
flowchart LR
  ...
  BOOKING --> ADAPTER
  ADAPTER -->|"운영 경로"| PG["Supavisor transaction pooler\nDATABASE_URL · prepared=false"]
...
| `app/page.tsx` | 13개 운영 화면, 예약 Drawer, 업무 Modal, 상태 기반 CTA |
...
| `app/api/rate-limit.ts` | Vercel 전체 instance가 공유하는 HMAC key 기반 PostgreSQL/D1 원자 카운터 |
...
| `db/pms-database.ts` | D1 형태의 prepared statement API를 server-only PostgreSQL transaction 또는 D1로 변환 |
...
| `202607160002_pms_data_api.sql` | 과거 HTTPS SQL adapter | 과거 RPC 생성 이력; 현재는 `202607170008`에서 전부 삭제됨 |
...
| `202607170006_default_admin_identity.sql` | 과거 관리자 계정 전환 | 과거 기본 할당 이력; 자동 생성된 ID는 다음 migration에서 삭제되고 현재 역할은 operator가 별도 provisioning |
...
| `rendered-html.test.mjs` | 제품 shell, 13개 화면, API action 존재, Toss font stylesheet/전역 적용, 모든 button의 action/submit/disabled와 전체 유지보수 코드 주석 계약 |
| `security-and-scale.test.mjs` | Supabase Auth, property scope, 500객실 atomic batch, FK·회계 경합, 보안 header, 직접 예약·홈페이지 CMS·Storage·WEB OFF 계약 |
| `security-remediation.test.mjs` | 관리자 seed/localhost 우회 부재, 금전 멱등성, 임의 SQL RPC 제거, D1 schema parity, staging QA gate, 분산 rate limit |
...
| Node test suite | 34/34 tests 통과, production build·TypeScript 포함 |
| 홈페이지 CMS E2E | 공개 projection, 동일 콘텐츠 version 저장, 잘못된 날짜 400, WEB OFF 제외·복원, Storage upload/public read/delete 통과 |
| Supabase smoke | 51 tables, 34 triggers, 79 validated FK, 51 RLS tables, 3개 공개 객실, Storage bucket·pooler 정상, 임의 SQL RPC 0 |
...
│  │  ├─ extended.ts           # Inventory/channel/accounting/website CMS service
...
│  ├─ hotel/                     # Dynamic public hotel website, safe search and direct booking UI
│  ├─ inventory-calendar.tsx   # 730-day bulk rate/inventory + direct-web visibility calendar
...
├─ db/
│  ├─ pms-database.ts          # D1/Postgres pooler adapter
│  └─ schema.ts                # Drizzle/D1 compatible schema
├─ scripts/
│  ├─ qa-full-workflow.mjs     # 전체 dummy workflow QA
│  ├─ qa-booking-engine.mjs    # 공개 예약·취소·멱등 E2E
│  ├─ qa-website-cms.mjs       # CMS projection·WEB OFF·Storage lifecycle E2E
│  ├─ qa-target.mjs            # Production URL/ref 차단 + staging health proof
...
- [ ] 별도 staging Supabase/Vercel 프로젝트와 staging Auth 사용자 provisioning
...
| Commit | 작업 |
| --- | --- |
| 2026-07-17 seven-finding security remediation | runtime/seed admin 제거, Host 기반 localhost 인증 제거, 금전 strict 멱등 receipt, 임의 SQL RPC 폐기와 pooler 전환, D1 booking schema parity, production QA hard gate, DB 분산 rate limit |
