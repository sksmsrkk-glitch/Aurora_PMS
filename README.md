...
2. Supabase Auth password login과 `/auth/v1/user` access token 검증
...
1. `Authorization: Bearer` 또는 `aurora-pms-access` cookie의 access token을 Supabase `/auth/v1/user`로 검증합니다.
...
7. 역할과 property assignment는 30초 cache하며 키는 `email + requested property`로 격리합니다.
...
- Supabase access token identity와 역할/property 할당 30초 short cache
