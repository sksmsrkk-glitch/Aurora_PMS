...
이 README는 빠른 시작과 운영 진입점입니다. 전체 화면 명세, 업무 규칙, 데이터 모델, API, 보안, QA 및 운영 절차는 [Aurora PMS 상세 개발·운영 핸드북](docs/aurora-pms-handbook.md)을 참조하세요.
...
| 데이터 | Supabase PostgreSQL 17, Supavisor, 54 tables, 52 tenant RLS policies |
| 스키마 계약 | `202607170012_rate_plan_domain` |
| 명령 계약 | 52개 action, capability 1:1, Zod 입력 검증, 멱등 mutation |
| 자동 검증 | lint, production build, 18 unit/behavior, 8 PostgreSQL integration |
...
- PMS 관리형 호텔 소개·이미지·객실 소개·공개 여부와 직접 예약 엔진
...
1. 앱을 우회 배포하지 않습니다.
2. 대상 DB의 `pms_schema_migrations`에서 `202607170012_rate_plan_domain`을 확인합니다.
...
- [전체 아키텍처·화면·업무·데이터·API·QA·운영 핸드북](docs/aurora-pms-handbook.md)
