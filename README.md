...
> 현재 운영 구성: Next.js/Vinext + Cloudflare Workers/Sites + Supabase PostgreSQL 17 + HTTPS Data API
...
```mermaid
flowchart LR
  U["호텔 운영자"] --> SIWC["Sites / Sign in with ChatGPT"]
  SIWC --> UI["Aurora Flow UI\nNext.js Client"]
...
1. Sites의 Sign in with ChatGPT와 owner/workspace 접근 정책
2. `oai-authenticated-user-email` 기반 사용자 식별
3. 서버 역할·capability 검증
4. 요청별 입력 검증과 상태 전이 검증
5. Supabase RLS 활성화와 browser role 권한 제거
6. 서버 전용 Secret Key로만 Data API 함수 호출
7. 감사 로그, reservation mutation, transition 기록
8. Idempotency-Key로 사용자 재클릭·네트워크 재시도 보호
9. 카드 원문·CVV 미저장
...
- HTTPS Data API를 통한 Workers 친화적 연결
...
- `SUPABASE_SECRET_KEY`: Workers 서버 런타임 전용
...
- [ ] `.env.local`과 Sites runtime env key 일치
