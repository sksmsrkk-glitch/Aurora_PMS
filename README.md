...
| 운영 스키마 | 50 tables, 34 triggers, 79 validated foreign keys, 50 RLS-enabled tables, 10 migrations |
| 자동 테스트 | production build + 26 tests + 홈페이지 CMS E2E |
...
운영 스키마는 50개 테이블을 11개 도메인으로 구성합니다. 79개 property-aware 외래키는 모두 validated 상태입니다.
...
PMS_BASE_URL=http://localhost:3000 npm run qa:website
```
...
| Supabase smoke | 50 tables, 34 triggers, 79 validated FK, 50 RLS tables, 3개 공개 객실, Storage bucket·Data API 정상 |
