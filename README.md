...
| 작업 브랜치 | `agent/connect-supabase-pms` |
| Draft PR | [#1 Expand Aurora PMS revenue, channel settlement, and accounting](https://github.com/sksmsrkk-glitch/Aurora_PMS/pull/1) |
| 기능 기준 커밋 | `aac1007` (`Use Toss Product Sans across PMS`); 이 README의 상세 문서는 현재 branch HEAD |
| 데이터베이스 | Supabase 프로젝트 `tnbxreeidezidckemflb`, PostgreSQL 17 |
| Vercel 함수 리전 | Mumbai `bom1` — Supabase `ap-south-1` 데이터 플레인과 인접 배치 |
| 운영 스키마 | 45 tables, 32 triggers, 45 RLS-enabled tables |
| 자동 테스트 | production build + 18 tests |
| 전체 업무 QA | local 24/24, Vercel production 24/24 workflow checkpoints |
| 성능 release gate | 300 requests, concurrency 30, 0 failures, 153.27 req/s, p95 207.86ms |
...
운영 스키마는 45개 테이블을 9개 도메인으로 구성합니다.
...
1. 신뢰 가능한 플랫폼의 `oai-authenticated-user-email` 헤더를 우선 사용합니다.
2. `localhost`/`127.0.0.1`은 개발 편의를 위해 `frontdesk@aurora.hotel`을 사용합니다.
3. 헤더가 없으면 명시적인 `PMS_DEMO_USER_EMAIL`을 사용합니다.
4. 어떤 사용자도 결정되지 않으면 `401`을 반환합니다.
5. `role_assignments`에서 활성 역할을 조회하고 30초 동안 역할만 cache합니다.
6. action에 연결된 capability가 없거나 사용자에게 capability가 없으면 `403`입니다.

> `PMS_DEMO_USER_EMAIL`은 인증이 아니라 테스트 identity fallback입니다. 인터넷에 공개된 실제 호텔 운영에서는 반드시 제거해야 합니다.
...
- 생성·전기·지급처럼 재시도 가능한 명령에는 고유한 `Idempotency-Key`를 보냅니다.
...
- `Accept-Encoding: gzip` 클라이언트에는 캐시된 운영 Snapshot을 gzip으로 전송해 대시보드 전송량 절감
...
- 벌크 변경을 Data API 한도에 맞춰 최대 450 statement 단위로 안전 분할

`npm run benchmark`는 전체 운영 Snapshot을 대상으로 warm-up 30회 후 300요청/동시성 30 조건을 검증하며, p95 250ms 미만과 오류 0건을 release gate로 사용합니다. 2026-07-16 로컬 release gate 측정은 0건 실패, 153.27 req/s, p95 207.86ms였습니다. 쓰기 직후에는 관련 cache를 즉시 비우므로 사용자 작업 결과가 오래된 Snapshot에 가려지지 않습니다.
...
| Vercel Production + 실제 Supabase | 24/24 checkpoints 통과 |
| Supabase smoke | 45 tables, 32 triggers, 45 RLS tables, Data API 정상 |
| font delivery | page 200, Toss stylesheet 200, WOFF2 200, Aurora Origin에 CORS `*` |

### Loop QA 범위

아래 25개 요구사항은 실행 스크립트에서 관련 업무를 묶어 24개의 checkpoint로 보고합니다.
...
│  ├─ chatgpt-auth.ts           # Trusted auth header helpers
...
├─ .openai/hosting.json        # legacy Sites metadata, Vercel runtime에는 미사용
└─ worker/index.ts
```
...
2. `/api/pms` 응답 status, `x-vercel-id`, cold/warm latency를 기록합니다.
3. Supabase Project Health와 Data API 상태를 확인합니다.
4. `npm run db:supabase:smoke`로 `pms_execute`, `pms_batch`, RLS, trigger를 분리 검증합니다.
5. Secret 환경 변수 이름이 `SUPABASE_URL`, `SUPABASE_SECRET_KEY`와 정확히 일치하는지 확인합니다. 값 자체를 log에 출력하지 않습니다.
6. 최근 migration이 실패했다면 migration history와 실제 catalog를 비교하고 새 corrective migration을 작성합니다.
7. 애플리케이션 회귀라면 Vercel에서 직전 정상 deployment를 promote합니다. DB migration은 애플리케이션 rollback과 자동으로 되돌아가지 않습니다.
...
- [ ] `PMS_DEMO_USER_EMAIL` 제거
- [ ] 호텔 SSO/OIDC 또는 서명 검증된 인증 프록시 연결
- [ ] 사용자 초대·비활성화·비밀번호/세션 정책 구축
- [ ] property 선택과 tenant context를 도입해 현재 `prop-seoul` 단일 property runtime 제거
...
- DB schema 대부분은 `property_id`를 보유하지만 API runtime은 현재 `prop-seoul` 단일 property를 기준으로 합니다.
...
| Commit | 작업 |
| --- | --- |
| 현재 README commit | 1,300줄 이상의 아키텍처·API·DB·개발·테스트·운영 인수인계 문서와 완전성 회귀 테스트 |
...
모든 기능 변경은 `agent/connect-supabase-pms` 브랜치와 Draft PR #1에 누적되어 있습니다. 운영 URL은 마지막 수동 production deployment의 커밋을 가리키며, PR merge는 별도 리뷰와 승인 후 수행합니다.
