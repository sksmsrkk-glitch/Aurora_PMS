/** Dependency-light OpenXML workbook builder shared by browser and server exports. */
...
export const safeSheetName=(value:string)=>value.replace(/[\\/?*:[\]]/g," ").slice(0,31)||"Report";
...
export function buildReportWorkbook(report:ExportReport){
  const parameters:Cell[][]=[
    [{value:"Talos PMS 리포트 내보내기",style:1},{value:"",style:1}],
    [{value:"리포트"},{value:report.title}],[{value:"생성 시각"},{value:report.generatedAt}],[{value:"내보내기 ID"},{value:report.exportId||""}],
    ...Object.entries(report.filters).map(([key,value])=>[{value:key},{value:value||"전체"}]),[{value:"결과 행 수"},{value:report.rows.length,style:3}],[],
    [{value:"요약",style:1},{value:"값",style:1}],...report.summary.map(item=>[{value:item.label},{value:item.value,style:item.format==="currency"?2:item.format==="percent"?4:3}]),
  ];
  ...
  const files:Record<string,Uint8Array>={
    ...
    "docProps/core.xml":strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xml(report.title)}</dc:title><dc:creator>Talos PMS</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${new Date().toISOString()}</dcterms:created></cp:coreProperties>`),
    "docProps/app.xml":strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>Talos PMS</Application><Company>주식회사 올마이투어</Company></Properties>`),
    "xl/workbook.xml":strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${sheetNames.map((name,index)=>`<sheet name="${xml(name)}" sheetId="${index+1}" r:id="rId${index+1}"/>`).join("")}</sheets></workbook>`),
    ...
    "xl/styles.xml":strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="#,##0.00"/><numFmt numFmtId="165" formatCode="0.00\&quot;%\&quot;"/></numFmts><fonts count="2"><font><sz val="10"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Aptos Display"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF103B32"/></patternFill></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf/></cellStyleXfs><cellXfs count="5"><xf/><xf fontId="1" fillId="2" applyFont="1" applyFill="1"/><xf numFmtId="164" applyNumberFormat="1"/><xf numFmtId="3" applyNumberFormat="1"/><xf numFmtId="165" applyNumberFormat="1"/></cellXfs></styleSheet>`),
    "xl/worksheets/sheet1.xml":strToU8(worksheet(parameters,false,false)),"xl/worksheets/sheet2.xml":strToU8(worksheet(data,true,true)),
  };
