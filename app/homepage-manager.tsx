...
type Media = { id:string;scope:"HOTEL"|"ROOM_TYPE";room_type_id:string|null;role:"HERO"|"GALLERY"|"CARD";public_url:string;alt_text:string;sort_order:number;active:boolean };
...
export default function HomepageManager({canAdmin}:{canAdmin:boolean}) {
  ...
  return <section className="website-manager">
    <div className="website-hero-panel"><div><p className="eyebrow">HOTEL WEBSITE STUDIO</p><h2>호텔 홈페이지 비주얼 에디터</h2><span>히어로·메뉴·호텔 소개·객실·이미지를 한곳에서 편집하고 공식 홈페이지에 반영합니다.</span></div><a className="primary" href="/hotel" target="_blank" rel="noreferrer">실제 홈페이지 열기 ↗</a></div>
    ...
    {tab==="editor"&&draft&&<div className="website-editor-layout">
      <section className="panel website-editor-controls">
        ...
        <details className="website-editor-group" open><summary><span><b>히어로</b><small>첫 화면 이미지와 메시지</small></span><i>⌄</i></summary><div className="website-editor-body">
          ...
          <div className="website-hero-picker">
            {hotelMedia.map(item=><button type="button" className={item.id===draft.heroMediaId?"on":""} key={item.id} onClick={()=>updateDraft("heroMediaId",item.id)} aria-label={`${item.alt_text} 히어로로 선택`}><i style={{backgroundImage:cssUrl(item.public_url)}}/><span>{item.alt_text}</span></button>)}
      ...
      <aside className="website-live-preview">
        <div className="website-preview-toolbar"><div><i/><i/><i/></div><span>hotel.example</span><div className="website-preview-devices"><button type="button" className={previewDevice==="desktop"?"on":""} onClick={()=>setPreviewDevice("desktop")} aria-label="데스크톱 미리보기">▱</button><button type="button" className={previewDevice==="mobile"?"on":""} onClick={()=>setPreviewDevice("mobile")} aria-label="모바일 미리보기">▯</button></div></div>
        <div className={`website-preview-canvas ${previewDevice}`} style={{"--preview-accent":draft.themeAccent} as CSSProperties}>
          <div className="website-preview-page">
            <header><b>{draft.hotelName}</b><nav>{draft.navigation.filter(item=>item.enabled).map(item=><span key={item.id}>{item.label}</span>)}</nav><i>{draft.bookingCtaLabel}</i></header>
            <section className={`website-preview-hero ${draft.heroLayout.toLowerCase()}`} style={{minHeight:`${Math.round(draft.heroHeight*.45)}px`,backgroundImage:selectedHero?`linear-gradient(90deg,rgba(5,13,29,${Math.min(.94,draft.heroOverlay/100+.14)}),rgba(5,13,29,${Math.max(.08,draft.heroOverlay/250)})),${cssUrl(selectedHero.public_url)}`:undefined}}>
