...
SELECT
  (SELECT COUNT(*) FROM rooms WHERE property_id=pms_current_property_id() AND active=1) AS rooms,
...
export async function snapshot(db: D1, principal?: Principal | null) {
  ...
  const activeRooms=rooms.filter(x=>Number(x.active??1)===1); const metrics=dashboardMetrics(comparisonResult.results[0] as Record<string,unknown>|undefined,activeRooms);
