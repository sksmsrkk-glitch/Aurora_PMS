...
export async function GET(request:Request){
  const resolved=await context(request);if("rejected" in resolved)return resolved.rejected;
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(parsed.data.action==="commit")return commit(resolved.db,resolved.principal.email,parsed.data.jobId);
    return rollback(resolved.db,resolved.principal.email,parsed.data.jobId);
