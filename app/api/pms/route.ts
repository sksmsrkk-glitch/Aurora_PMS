...
async function processChannelMessage(db:D1,message:Record<string,unknown>,payload:ChannelPayload,actor:string,now:string) {
  ...
  if(eventType==="NEW"){
    if(link)throw new Error("channel reservation already linked");const mapping=await db.prepare("SELECT * FROM channel_mappings WHERE connection_id=? AND external_room_type_id=? AND external_rate_plan_id=? AND property_id=pms_current_property_id() AND active=1").bind(payload.connectionId,payload.externalRoomTypeId,payload.externalRatePlanId).first<Record<string,unknown>>();if(!mapping)throw new Error("channel mapping unavailable");
    ...
    if(!link||link.status!=="ACTIVE")throw new Error("channel reservation link unavailable");const reservation=await db.prepare("SELECT * FROM reservations WHERE id=? AND property_id=pms_current_property_id() AND status='DUE_IN'").bind(link.reservation_id).first<Record<string,unknown>>();if(!reservation)throw new Error("channel reservation cannot be modified");const mapping=await db.prepare("SELECT * FROM channel_mappings WHERE connection_id=? AND external_room_type_id=? AND external_rate_plan_id=? AND property_id=pms_current_property_id() AND active=1").bind(payload.connectionId,payload.externalRoomTypeId,payload.externalRatePlanId).first<Record<string,unknown>>();if(!mapping||!payload.arrivalDate||!payload.departureDate)throw new Error("channel mapping unavailable");const controlError=await stayControlError(db,String(mapping.room_type_id),payload.arrivalDate,payload.departureDate);if(controlError)throw new Error(controlError);reservationId=String(link.reservation_id);
...
async function snapshot(db: D1, principal?: Principal | null) {
  ...
  const [propertyResult,reservationResult,roomResult,actorCashierResult,openCashierResult,failedResult,auditResult,postingsResult,roomTypesResult,typeNightsResult,inventoryControlsResult,accountProfilesResult,blocksResult,blockInventoryResult,roomingResult,folioWindowsResult,folioEntriesResult,routingRulesResult,transactionCodesResult,arAccountsResult,arInvoicesResult,trialBalanceResult,channelConnectionsResult,channelContractsResult,channelMappingsResult,ariUpdatesResult,inboundMessagesResult,channelLinksResult,integrationAttemptsResult,outboxResult] = await db.batch([
    ...
    principal ? db.prepare("SELECT * FROM cashier_sessions WHERE property_id=pms_current_property_id() AND actor=? AND status='OPEN' ORDER BY opened_at DESC LIMIT 1").bind(principal.email) : db.prepare("SELECT * FROM cashier_sessions WHERE 0"),
    ...
    db.prepare("SELECT room_type_id, stay_date, COUNT(*) booked FROM reservation_type_nights WHERE property_id=pms_current_property_id() AND stay_date BETWEEN (SELECT business_date FROM properties WHERE id=pms_current_property_id()) AND to_char(((SELECT business_date FROM properties WHERE id=pms_current_property_id())::date + INTERVAL '13 day'),'YYYY-MM-DD') GROUP BY room_type_id, stay_date"),
    db.prepare("SELECT * FROM inventory_controls WHERE property_id=pms_current_property_id() AND stay_date BETWEEN (SELECT business_date FROM properties WHERE id=pms_current_property_id()) AND to_char(((SELECT business_date FROM properties WHERE id=pms_current_property_id())::date + INTERVAL '13 day'),'YYYY-MM-DD')"),
    db.prepare("SELECT * FROM account_profiles WHERE property_id=pms_current_property_id() AND active=1 ORDER BY type,name"),
    ...
    db.prepare("SELECT rr.*,w.window_no,w.name window_name,r.confirmation_no FROM folio_routing_rules rr JOIN folio_windows w ON w.id=rr.target_window_id JOIN reservations r ON r.id=rr.reservation_id WHERE rr.property_id=pms_current_property_id() AND rr.active=1 ORDER BY rr.created_at DESC"),
    db.prepare("SELECT * FROM transaction_codes WHERE property_id=pms_current_property_id() AND active=1 ORDER BY category,code"),
  ...
  if(body.action==="export_report") {
    try {
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'EXPORT_REPORT', 'report_export', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,exportId,JSON.stringify({report:report.report.key,filters,rowCount:report.rows.length,format}),now),
        mutationReceipt(),
      ]);
      invalidateSnapshots();
  ...
  try {
    if(await handleExtendedAction(db,body,principal,businessDate,now,idempotencyKey)) {
      ...
      const typeId=crypto.randomUUID();await db.batch([
        db.prepare("INSERT INTO room_types(id,property_id,code,name,base_rate,capacity,description,active) VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, 1)").bind(typeId,code,name,baseRate,capacity,description),
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_ROOM_TYPE', 'room_type', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,typeId,JSON.stringify({code,name,baseRate,capacity,description,active:true}),now),
      ...
      await db.batch([db.prepare("UPDATE room_types SET code=?,name=?,base_rate=?,capacity=?,description=?,active=?,version=version+1 WHERE id=? AND property_id=pms_current_property_id() AND version=?").bind(code,name,baseRate,capacity,description,active?1:0,body.roomTypeId,Number(current.version)),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'UPDATE_ROOM_TYPE', 'room_type', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.roomTypeId,JSON.stringify(current),JSON.stringify({code,name,baseRate,capacity,description,active,version:Number(current.version)+1}),now),...(idempotencyKey?[db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)]:[])]);
    } else if(body.action==="create_room") {
      const number=(body.number||"").trim().toUpperCase(),floor=Number(body.floor),type=await db.prepare("SELECT id FROM room_types WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomTypeId).first(),features=(body.features||"").split(",").map(value=>value.trim()).filter(Boolean).slice(0,20);
      ...
      await db.batch([db.prepare("INSERT INTO rooms(id,property_id,room_type_id,number,floor,front_desk_status,housekeeping_status,features,active,version) VALUES (?, pms_current_property_id(), ?, ?, ?, 'VACANT', 'CLEAN', ?, 1, 1)").bind(roomId,body.roomTypeId,number,floor,JSON.stringify(features)),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_ROOM', 'room', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,roomId,JSON.stringify({number,floor,roomTypeId:body.roomTypeId,features}),now),...(idempotencyKey?[db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)]:[])]);
    } else if(body.action==="bulk_create_rooms") {
      const start=Number(body.startNumber),count=Number(body.count),floor=Number(body.floor),padding=Math.min(8,Math.max(1,Number(body.padding)||String(body.startNumber||"").length)),prefix=(body.prefix||"").trim().toUpperCase().slice(0,8),type=await db.prepare("SELECT id FROM room_types WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomTypeId).first(),features=(body.features||"").split(",").map(value=>value.trim()).filter(Boolean).slice(0,20);
      ...
      const roomStatements=numbers.map(number=>db.prepare("INSERT INTO rooms(id,property_id,room_type_id,number,floor,front_desk_status,housekeeping_status,features,active,version) VALUES (?, pms_current_property_id(), ?, ?, ?, 'VACANT', 'CLEAN', ?, 1, 1)").bind(crypto.randomUUID(),body.roomTypeId,number,floor,JSON.stringify(features)));
      await db.batch([...roomStatements,db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'BULK_CREATE_ROOMS', 'room_batch', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,crypto.randomUUID(),JSON.stringify({roomTypeId:body.roomTypeId,prefix,start,count,floor,numbers}),now),db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)]);
    } else if(body.action==="update_room") {
      const current=await db.prepare("SELECT * FROM rooms WHERE id=? AND property_id=pms_current_property_id()").bind(body.roomId).first<Record<string,unknown>>(),number=(body.number||"").trim().toUpperCase(),floor=Number(body.floor),active=body.active!=="false",type=await db.prepare("SELECT id FROM room_types WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomTypeId).first(),features=(body.features||"").split(",").map(value=>value.trim()).filter(Boolean).slice(0,20);
      ...
      await db.batch([db.prepare("UPDATE rooms SET room_type_id=?,number=?,floor=?,features=?,active=?,housekeeping_status=?,version=version+1 WHERE id=? AND property_id=pms_current_property_id() AND version=?").bind(body.roomTypeId,number,floor,JSON.stringify(features),active?1:0,housekeeping,body.roomId,Number(current.version)),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'UPDATE_ROOM', 'room', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.roomId,JSON.stringify(current),JSON.stringify({roomTypeId:body.roomTypeId,number,floor,features,active,housekeeping,version:Number(current.version)+1}),now),...(idempotencyKey?[db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)]:[])]);
      ...
      const type = await db.prepare("SELECT * FROM room_types WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomTypeId).first<Record<string,unknown>>();
      ...
      const room = body.roomId ? await db.prepare("SELECT * FROM rooms WHERE id=? AND room_type_id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomId,body.roomTypeId).first<Record<string,unknown>>() : null;
      ...
      const statements = [
        ...
        db.prepare("INSERT INTO reservations VALUES (?, ?, pms_current_property_id(), ?, ?, ?, ?, ?, 'DUE_IN', ?, ?, ?, ?, ?, ?, '', 1, ?, ?)").bind(reservationId,confirmation,guestId,body.roomTypeId,body.roomId||null,body.arrivalDate,body.departureDate,Number(body.adults)||1,Number(body.children)||0,body.source||"Direct",body.ratePlan||"BAR",Number(body.nightlyRate)||Number(type.base_rate),body.eta||null,now,now),
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_RESERVATION', 'reservation', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,reservationId,JSON.stringify({confirmation,status:"DUE_IN"}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'reservation.created', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),reservationId,JSON.stringify({reservationId,confirmation}),now));
      ...
      const type=await db.prepare("SELECT * FROM room_types WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomTypeId).first<Record<string,unknown>>(); if(!type) return Response.json({error:"객실 타입이 올바르지 않습니다."},{status:400});
      ...
      const statements:D1PreparedStatement[]=[
        ...
        db.prepare("UPDATE reservations SET room_type_id=?, room_id=?, arrival_date=?, departure_date=?, adults=?, children=?, rate_plan=?, nightly_rate=?, eta=?, notes=?, version=version+1, updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND status='DUE_IN' AND version=?").bind(body.roomTypeId,retainedRoom,body.arrivalDate,body.departureDate,Math.max(1,Number(body.adults)||1),Math.max(0,Number(body.children)||0),body.ratePlan||String(reservation.rate_plan),Number(body.nightlyRate)||Number(type.base_rate),body.eta||null,body.notes||"",now,body.reservationId,expectedVersion),
      ];
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'EDIT_RESERVATION', 'reservation', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify(reservation),JSON.stringify({roomTypeId:body.roomTypeId,arrivalDate:body.arrivalDate,departureDate:body.departureDate,roomId:retainedRoom}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'reservation.updated', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,version:expectedVersion+1}),now));
      if(idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const expectedVersion=Number(body.expectedVersion); if(expectedVersion!==Number(reservation.version)) return Response.json({error:"다른 작업자가 예약을 변경했습니다. 화면을 새로고침하세요."},{status:409});
      const room=await db.prepare("SELECT * FROM rooms WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.roomId).first<Record<string,unknown>>();
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'ASSIGN_ROOM', 'reservation', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify({roomId:reservation.room_id}),JSON.stringify({roomId:body.roomId}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'reservation.room_assigned', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,roomId:body.roomId}),now));
      if(idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'MOVE_ROOM', 'reservation', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify({roomId:reservation.room_id}),JSON.stringify({roomId:body.roomId,reason:body.reason}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'stay.room_moved', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,fromRoomId:reservation.room_id,toRoomId:body.roomId,reason:body.reason}),now));
      if(idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const statements:D1PreparedStatement[]=[
        ...
        db.prepare("DELETE FROM reservation_nights WHERE reservation_id=? AND property_id=pms_current_property_id()").bind(body.reservationId),
        db.prepare("DELETE FROM reservation_type_nights WHERE reservation_id=? AND property_id=pms_current_property_id()").bind(body.reservationId),
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CANCEL_RESERVATION', 'reservation', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify(reservation),JSON.stringify({status:"CANCELLED",reason:body.reason.trim()}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'reservation.cancelled', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,reason:body.reason.trim()}),now),
      ];
      ...
      const capacity=await db.prepare("SELECT COUNT(*) count FROM rooms WHERE property_id=pms_current_property_id() AND room_type_id=? AND active=1 AND housekeeping_status<>'OUT_OF_SERVICE'").bind(body.roomTypeId).first<{count:number}>(); const physical=Number(capacity?.count??0);
      ...
      const statements:D1PreparedStatement[]=[
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'UPDATE_INVENTORY_CONTROL', 'inventory_control', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,controlId,existing?JSON.stringify(existing):null,JSON.stringify({roomTypeId:body.roomTypeId,stayDate,sellLimit,closed:Boolean(closed),websiteClosed:Boolean(websiteClosed),minStay,cta:body.cta==="true",ctd:body.ctd==="true",price}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'inventory.updated', 'room_type', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.roomTypeId,JSON.stringify({roomTypeId:body.roomTypeId,stayDate,sellLimit,closed:Boolean(closed),minStay,price}),now),
      ];
      if(idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const profileId=crypto.randomUUID(); const statements:D1PreparedStatement[]=[
        db.prepare("INSERT INTO account_profiles VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?)").bind(profileId,type,name,body.externalId?.trim()||null,body.email?.trim()||null,body.phone?.trim()||null,body.negotiatedRateCode?.trim()||null,body.creditStatus||"CASH",body.notes||"",now,now),
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_ACCOUNT_PROFILE', 'account_profile', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,profileId,JSON.stringify({type,name,externalId:body.externalId||null}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'profile.created', 'account_profile', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),profileId,JSON.stringify({profileId,type,name}),now),
      ];
      ...
      const accountId=body.accountProfileId||null,groupId=body.groupProfileId||null; if(accountId&&!await db.prepare("SELECT id FROM account_profiles WHERE id=? AND property_id=pms_current_property_id() AND type IN ('COMPANY','TRAVEL_AGENT','SOURCE') AND active=1").bind(accountId).first()) return Response.json({error:"유효한 회사·여행사·소스 프로필을 선택하세요."},{status:400}); if(groupId&&!await db.prepare("SELECT id FROM account_profiles WHERE id=? AND property_id=pms_current_property_id() AND type='GROUP' AND active=1").bind(groupId).first()) return Response.json({error:"유효한 그룹 프로필을 선택하세요."},{status:400});
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_BUSINESS_BLOCK', 'business_block', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,blockId,JSON.stringify({code,name:body.name,status,arrivalDate:body.arrivalDate,departureDate:body.departureDate,allocations}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'block.created', 'business_block', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),blockId,JSON.stringify({blockId,code,status}),now));
      if(idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)); await db.batch(statements);
      ...
      const statements:D1PreparedStatement[]=[db.prepare("UPDATE block_inventory SET current_rooms=?,rate=?,version=version+1,updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND version=?").bind(rooms,rate,now,row.id,Number(row.version)),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'UPDATE_BLOCK_INVENTORY', 'block_inventory', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,String(row.id),JSON.stringify(row),JSON.stringify({currentRooms:rooms,rate}),now),db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'block.inventory_updated', 'business_block', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.blockId,JSON.stringify({blockId:body.blockId,roomTypeId:body.roomTypeId,stayDate:body.stayDate,rooms,rate}),now)]; if(idempotencyKey)statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)); await db.batch(statements);
      ...
      const entryId=crypto.randomUUID(),rate=Number(body.rate)||Number(grid.results[0]?.rate??0); const statements:D1PreparedStatement[]=[db.prepare("INSERT INTO rooming_list_entries VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', NULL, ?, ?, 1, ?, ?)").bind(entryId,body.blockId,body.firstName.trim(),body.lastName.trim(),body.email||null,body.phone||null,body.arrivalDate,body.departureDate,body.roomTypeId,rate,body.notes||"",now,now),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'ADD_ROOMING_ENTRY', 'rooming_list_entry', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,entryId,JSON.stringify({blockId:body.blockId,firstName:body.firstName,lastName:body.lastName,roomTypeId:body.roomTypeId}),now)]; if(idempotencyKey)statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)); await db.batch(statements);
      ...
      const reservationId=crypto.randomUUID(),guestId=crypto.randomUUID(),confirmation=`SEL-${String(entry.arrival_date).replaceAll("-","").slice(2)}-${Math.floor(1000+Math.random()*9000)}`,stayDates=datesBetween(String(entry.arrival_date),String(entry.departure_date)); const statements:D1PreparedStatement[]=[
        ...
        db.prepare("INSERT INTO reservations VALUES (?, ?, pms_current_property_id(), ?, ?, NULL, ?, ?, 'DUE_IN', 1, 0, 'Group', ?, ?, NULL, ?, 1, ?, ?)").bind(reservationId,confirmation,guestId,String(entry.room_type_id),String(entry.arrival_date),String(entry.departure_date),String(entry.block_code),Number(entry.rate),`Block ${entry.block_code} · Rooming list`,now,now),
        db.prepare("INSERT INTO folio_windows VALUES (?, pms_current_property_id(), ?, 1, 'Guest Folio', 'GUEST', NULL, 'OPEN', ?, ?, NULL)").bind(`fw-${reservationId}`,reservationId,now,actor),
      ];
      ...
      statements.push(db.prepare("UPDATE rooming_list_entries SET status='PICKED_UP',reservation_id=?,version=version+1,updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND status='PENDING'").bind(reservationId,now,String(entry.id))); statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'PICKUP_ROOMING_ENTRY', 'rooming_list_entry', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,String(entry.id),JSON.stringify(entry),JSON.stringify({status:"PICKED_UP",reservationId,confirmation}),now)); statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'block.reservation_picked_up', 'business_block', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),String(entry.block_id),JSON.stringify({blockId:entry.block_id,entryId:entry.id,reservationId,confirmation}),now)); if(idempotencyKey)statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)); await db.batch(statements);
    } else if (body.action === "cutoff_block") {
      const block=await db.prepare("SELECT * FROM business_blocks WHERE id=? AND property_id=pms_current_property_id() AND status IN ('TENTATIVE','DEFINITE')").bind(body.blockId).first<Record<string,unknown>>(); if(!block)return Response.json({error:"마감 가능한 블록을 찾지 못했습니다."},{status:409}); const statements:D1PreparedStatement[]=[db.prepare("UPDATE block_inventory SET current_rooms=picked_up,version=version+1,updated_at=? WHERE block_id=? AND property_id=pms_current_property_id()").bind(now,body.blockId),db.prepare("UPDATE business_blocks SET status='CUTOFF',cutoff_processed_at=?,version=version+1,updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND status IN ('TENTATIVE','DEFINITE')").bind(now,now,body.blockId),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CUTOFF_BLOCK', 'business_block', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.blockId,JSON.stringify(block),JSON.stringify({status:"CUTOFF"}),now),db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'block.cutoff', 'business_block', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.blockId,JSON.stringify({blockId:body.blockId}),now)];if(idempotencyKey)statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));await db.batch(statements);
      ...
      const connection=await db.prepare("SELECT id FROM channel_connections WHERE id=? AND property_id=pms_current_property_id() AND status='ACTIVE'").bind(body.connectionId).first(),roomType=await db.prepare("SELECT id FROM room_types WHERE id=? AND property_id=pms_current_property_id()").bind(body.roomTypeId).first();if(!connection||!roomType||!body.externalRoomTypeId?.trim()||!body.externalRatePlanId?.trim())return Response.json({error:"활성 연결, 객실 타입, 외부 room/rate ID를 입력하세요."},{status:400});const mappingId=crypto.randomUUID();await db.batch([db.prepare("INSERT INTO channel_mappings VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, 1, ?, ?)").bind(mappingId,body.connectionId,body.roomTypeId,body.externalRoomTypeId.trim(),body.ratePlan||"OTA",body.externalRatePlanId.trim(),now,now),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_CHANNEL_MAPPING', 'channel_mapping', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,mappingId,JSON.stringify({connectionId:body.connectionId,roomTypeId:body.roomTypeId,externalRoomTypeId:body.externalRoomTypeId,externalRatePlanId:body.externalRatePlanId}),now)]);
    } else if (body.action === "queue_ari_delta") {
      const mapping=await db.prepare("SELECT m.*,c.provider FROM channel_mappings m JOIN channel_connections c ON c.id=m.connection_id WHERE m.id=? AND m.property_id=pms_current_property_id() AND c.property_id=pms_current_property_id() AND m.active=1 AND c.status='ACTIVE'").bind(body.mappingId).first<Record<string,unknown>>(),dates=datesBetween(body.startDate,(()=>{const end=new Date(`${body.endDate}T00:00:00Z`);end.setUTCDate(end.getUTCDate()+1);return end.toISOString().slice(0,10)})());if(!mapping||!dates.length)return Response.json({error:"활성 매핑과 올바른 ARI 일자 범위를 선택하세요."},{status:400});const physicalRow=await db.prepare("SELECT COUNT(*) count,MAX(rt.base_rate) base_rate FROM rooms r JOIN room_types rt ON rt.id=r.room_type_id WHERE r.property_id=pms_current_property_id() AND r.room_type_id=? AND r.active=1 AND r.housekeeping_status<>'OUT_OF_SERVICE'").bind(mapping.room_type_id).first<{count:number;base_rate:number}>(),statements:D1PreparedStatement[]=[];
      for(const stayDate of dates){const [control,booked,held,prior]=await Promise.all([db.prepare("SELECT * FROM inventory_controls WHERE property_id=pms_current_property_id() AND room_type_id=? AND stay_date=?").bind(mapping.room_type_id,stayDate).first<Record<string,unknown>>(),db.prepare("SELECT COUNT(*) count FROM reservation_type_nights WHERE property_id=pms_current_property_id() AND room_type_id=? AND stay_date=?").bind(mapping.room_type_id,stayDate).first<{count:number}>(),db.prepare("SELECT COALESCE(SUM(bi.current_rooms-bi.picked_up),0) count FROM block_inventory bi JOIN business_blocks bb ON bb.id=bi.block_id AND bb.property_id=bi.property_id WHERE bi.property_id=pms_current_property_id() AND bi.room_type_id=? AND bi.stay_date=? AND bb.deduct_inventory=1 AND bb.status IN ('TENTATIVE','DEFINITE')").bind(mapping.room_type_id,stayDate).first<{count:number}>(),db.prepare("SELECT COALESCE(MAX(revision),0)+1 revision FROM ari_updates WHERE mapping_id=? AND property_id=pms_current_property_id() AND stay_date=?").bind(mapping.id,stayDate).first<{revision:number}>()]);const sellLimit=control?.sell_limit==null?Number(physicalRow?.count??0):Number(control.sell_limit),available=Boolean(control?.closed)?0:Math.max(0,sellLimit-Number(booked?.count??0)-Number(held?.count??0)),revision=Number(prior?.revision??1),payload={roomstosell:available,closed:Boolean(control?.closed),minimumstay:Number(control?.min_stay??1),closedonarrival:Boolean(control?.close_to_arrival),closedondeparture:Boolean(control?.close_to_departure),rate:Number(control?.price_override??physicalRow?.base_rate??0),currency:"KRW",date:stayDate};const ariId=crypto.randomUUID();statements.push(db.prepare("INSERT INTO ari_updates VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'KRW', ?, 'PENDING', 0, ?, NULL, NULL)").bind(ariId,mapping.connection_id,mapping.id,stayDate,revision,available,payload.closed?1:0,payload.minimumstay,payload.closedonarrival?1:0,payload.closedondeparture?1:0,payload.rate,JSON.stringify(payload),now));statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'channel.ari_delta', 'ari_update', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),ariId,JSON.stringify(payload),now));}
      await db.batch(statements);
      ...
      const cashierId = crypto.randomUUID(); const statements = [
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'OPEN_CASHIER', 'cashier_session', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,cashierId,JSON.stringify({openingAmount,businessDate:property?.business_date}),now),
      ];
      if (idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const variance = counted-expected; const statements = [
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CLOSE_CASHIER', 'cashier_session', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,session.id,JSON.stringify(session),JSON.stringify({status:"CLOSED",expected,counted,variance}),now),
      ];
      if (idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const statements = [db.prepare("INSERT INTO night_audits VALUES (?, pms_current_property_id(), ?, 'COMPLETED', '[]', ?, ?, ?, ?)").bind(auditId,businessDate,JSON.stringify({roomPostings:stays.results.length,blockCutoffs:cutoffBlocks.results.length,nextBusinessDate:nextDate}),now,now,actor)];
      ...
      for(const block of cutoffBlocks.results){statements.push(db.prepare("UPDATE block_inventory SET current_rooms=picked_up,version=version+1,updated_at=? WHERE block_id=? AND property_id=pms_current_property_id()").bind(now,block.id));statements.push(db.prepare("UPDATE business_blocks SET status='CUTOFF',cutoff_processed_at=?,version=version+1,updated_at=? WHERE id=? AND property_id=pms_current_property_id()").bind(now,now,block.id));statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'block.cutoff', 'business_block', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),block.id,JSON.stringify({blockId:block.id,automatic:true,businessDate}),now));}
      ...
      statements.push(db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CLOSE_BUSINESS_DATE', 'night_audit', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,auditId,JSON.stringify({businessDate}),JSON.stringify({nextDate,roomPostings:stays.results.length,blockCutoffs:cutoffBlocks.results.length}),now));
      statements.push(db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'business_date.closed', 'night_audit', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),auditId,JSON.stringify({businessDate,nextDate}),now));
      if (idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      const statements = [
        ...
        db.prepare("DELETE FROM reservation_nights WHERE reservation_id=? AND property_id=pms_current_property_id()").bind(body.reservationId),
        db.prepare("DELETE FROM reservation_type_nights WHERE reservation_id=? AND property_id=pms_current_property_id()").bind(body.reservationId),
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'MARK_NO_SHOW', 'reservation', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify(reservation),JSON.stringify({status:"NO_SHOW"}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'reservation.no_show', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId}),now),
      ];
      if (idempotencyKey) statements.push(db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now));
      await db.batch(statements);
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,"CHECK_IN","reservation",body.reservationId,JSON.stringify(reservation),JSON.stringify({status:"IN_HOUSE"}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'stay.checked_in', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,roomId:reservation.room_id}),now),
        ...(idempotencyKey ? [db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)] : []),
      ]);
      ...
      await db.batch([
        ...
        db.prepare("UPDATE reservations SET status='CHECKED_OUT', departure_date=CASE WHEN departure_date>? THEN ? ELSE departure_date END, version=version+1, updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND status='IN_HOUSE'").bind(businessDate,businessDate,now,body.reservationId),
        db.prepare("DELETE FROM reservation_nights WHERE reservation_id=? AND property_id=pms_current_property_id() AND stay_date>=?").bind(body.reservationId,businessDate),
        db.prepare("DELETE FROM reservation_type_nights WHERE reservation_id=? AND property_id=pms_current_property_id() AND stay_date>=?").bind(body.reservationId,businessDate),
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,"CHECK_OUT","reservation",body.reservationId,JSON.stringify(reservation),JSON.stringify({status:"CHECKED_OUT"}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'stay.checked_out', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,roomId:reservation.room_id}),now),
        ...(idempotencyKey ? [db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)] : []),
      ]);
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'folio.payment_posted', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,amount,method:body.method||"CARD"}),now),
        ...(idempotencyKey ? [db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)] : []),
      ]);
      ...
      const code=(body.code||"MISC").toUpperCase(),transactionCode=await db.prepare("SELECT * FROM transaction_codes WHERE property_id=pms_current_property_id() AND code=? AND active=1").bind(code).first<Record<string,unknown>>();
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'folio.posted', 'reservation', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),body.reservationId,JSON.stringify({reservationId:body.reservationId,amount,kind:"CHARGE"}),now),
        ...(idempotencyKey ? [db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)] : []),
      ]);
      ...
      if(body.accountProfileId&&!await db.prepare("SELECT id FROM account_profiles WHERE id=? AND property_id=pms_current_property_id() AND active=1").bind(body.accountProfileId).first())return Response.json({error:"유효한 계정 프로필을 선택하세요."},{status:400});
      await db.batch([db.prepare("INSERT INTO folio_windows VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, 'OPEN', ?, ?, NULL)").bind(windowId,body.reservationId,Number(next?.next_no??1),body.name?.trim()||`Window ${next?.next_no??1}`,payeeType,body.accountProfileId||null,now,actor),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_FOLIO_WINDOW', 'reservation', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify({windowId,payeeType}),now),mutationReceipt()]);
      ...
      await db.batch([db.prepare("INSERT INTO folio_routing_rules VALUES (?, pms_current_property_id(), ?, ?, ?, 1, ?, ?) ON CONFLICT(reservation_id,transaction_code) DO UPDATE SET target_window_id=excluded.target_window_id,active=1").bind(crypto.randomUUID(),body.reservationId,code,body.windowId,now,actor),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'UPSERT_FOLIO_ROUTING', 'reservation', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,body.reservationId,JSON.stringify({code,windowId:body.windowId}),now),mutationReceipt()]);
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'SPLIT_FOLIO_ENTRY', 'folio_entry', ?, ?, ?, ?)").bind(crypto.randomUUID(),actor,String(source.id),JSON.stringify(source),JSON.stringify({amount,targetWindowId:body.targetWindowId,reverseId,repostId,reason}),now),
        mutationReceipt(),
      ]);
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'TRANSFER_FOLIO_TO_AR', 'ar_invoice', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,invoiceId,JSON.stringify({invoiceNo,amount,windowId:window.id,accountProfileId:profile.id}),now),
        db.prepare("INSERT INTO outbox_events VALUES (?, pms_current_property_id(), 'ar.invoice_issued', 'ar_invoice', ?, ?, 'PENDING', 0, ?, NULL)").bind(crypto.randomUUID(),invoiceId,JSON.stringify({invoiceId,invoiceNo,amount}),now),
        mutationReceipt(),
      ]);
      ...
      await db.batch([
        ...
        db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, ?, ?, ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,"HOUSEKEEPING_COMPLETE","room",body.roomId,JSON.stringify({housekeepingStatus:status}),now),
        ...(idempotencyKey ? [db.prepare("INSERT INTO idempotency_keys VALUES (?, pms_current_property_id(), ?, ?, ?)").bind(idempotencyKey,body.action,actor,now)] : []),
      ]);
