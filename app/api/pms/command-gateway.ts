...
async function processChannelMessage(db:D1,message:Record<string,unknown>,payload:ChannelPayload,actor:string,now:string) {
  ...
  if(eventType==="NEW"){
    ...
    statements.push(db.prepare("INSERT INTO reservations(id,confirmation_no,property_id,guest_id,room_type_id,room_id,arrival_date,departure_date,status,adults,children,source,rate_plan,nightly_rate,eta,notes,version,created_at,updated_at,booker_name,booker_email,channel_product_name,payment_type) VALUES (?, ?, pms_current_property_id(), ?, ?, NULL, ?, ?, 'DUE_IN', ?, ?, ?, ?, ?, NULL, ?, 1, ?, ?, ?, ?, ?, 'CHANNEL')").bind(reservationId,confirmation,guestId,mapping.room_type_id,payload.arrivalDate,payload.departureDate,Number(payload.adults)||1,Number(payload.children)||0,String(connection.provider),String(mapping.rate_plan),nightlyRate,`Channel ${payload.externalReservationId} · revision ${revision}`,now,now,`${payload.firstName.trim()} ${payload.lastName.trim()}`,payload.email||null,String(mapping.rate_plan)));
...
export async function handlePmsPost(request: Request) {
  ...
  try {
    if(ROOM_ASSIGNMENT_ACTIONS.has(body.action as never)){
    ...
    } else if(registration.domain==="accounting"||registration.domain==="website"||(registration.domain==="inventory"&&["bulk_update_inventory_controls","upsert_rate_plan","bulk_update_rate_blocks"].includes(body.action))||(registration.domain==="integrations"&&["upsert_channel_contract","upsert_channel_catalog","configure_property_channel","set_property_channel_active","reorder_property_channels","delete_property_channel","upsert_channel_product_cutoff","delete_channel_product_cutoff"].includes(body.action))||(registration.domain==="rooms"&&["upsert_property_season","delete_property_season","upsert_property_holiday","delete_property_holiday","upsert_amenity_catalog","delete_amenity_catalog","upsert_service_catalog","delete_service_catalog"].includes(body.action))) {
      ...
      const statements = [
        ...
        db.prepare("INSERT INTO reservations(id,confirmation_no,property_id,guest_id,room_type_id,room_id,arrival_date,departure_date,status,adults,children,source,rate_plan,rate_plan_id,nightly_rate,eta,notes,version,created_at,updated_at,booker_name,booker_phone,booker_email,channel_product_name,payment_type,guest_request) VALUES (?, ?, pms_current_property_id(), ?, ?, ?, ?, ?, 'DUE_IN', ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)").bind(reservationId,confirmation,guestId,body.roomTypeId,body.roomId||null,body.arrivalDate,body.departureDate,adults,children,source,requestedRatePlan,ratePlan.id,averageRate,body.eta||null,notes,now,now,(body.bookerName||`${body.firstName.trim()} ${body.lastName.trim()}`).trim(),body.bookerPhone||body.phone||null,body.bookerEmail||body.email||null,String(ratePlan.name||requestedRatePlan),body.paymentType||"HOTEL",body.guestRequest||""),
    ...
    } else if (body.action === "update_reservation_detail" && reservation) {
      const expectedVersion=Number(body.expectedVersion);
      if(expectedVersion!==Number(reservation.version))return Response.json({error:"다른 작업자가 예약 상세를 먼저 변경했습니다. 최신 내용을 확인해 주세요."},{status:409});
      const guest=await db.prepare("SELECT * FROM guests WHERE id=? AND property_id=pms_current_property_id()").bind(String(reservation.guest_id)).first<Record<string,unknown>>();
      const roomType=await db.prepare("SELECT capacity FROM room_types WHERE id=? AND property_id=pms_current_property_id()").bind(String(reservation.room_type_id)).first<{capacity:number}>();
      if(!guest||!roomType)return Response.json({error:"예약 고객 또는 객실 타입을 찾지 못했습니다."},{status:404});
      const bookerName=(body.bookerName||"").trim().slice(0,120),guestFirstName=(body.guestFirstName||"").trim().slice(0,80),guestLastName=(body.guestLastName||"").trim().slice(0,80);
      const adults=Number(body.adults),children=Number(body.children),paymentType=String(body.paymentType||"HOTEL");
      const asFlag=(value:unknown)=>value===true||value==="true",checked=asFlag(body.reservationChecked),early=asFlag(body.earlyCheckin),late=asFlag(body.lateCheckout);
      const earlyTime=early?String(body.earlyCheckinTime||""):null,lateTime=late?String(body.lateCheckoutTime||""):null;
      const email=(value:string)=>!value||/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(value);
      const cardRef=(body.cardInfoRef||"").trim().slice(0,160)||null;
      if(!bookerName||!guestFirstName||!guestLastName||!Number.isInteger(adults)||adults<1||!Number.isInteger(children)||children<0||adults+children>Number(roomType.capacity))return Response.json({error:"예약자·투숙자 이름과 객실 정원 내 인원을 확인하세요."},{status:400});
      if(!email(body.bookerEmail||"")||!email(body.guestEmail||""))return Response.json({error:"예약자와 투숙자 이메일 형식을 확인하세요."},{status:400});
      if(!["HOTEL","PREPAID","CHANNEL","DIRECT_BILL"].includes(paymentType))return Response.json({error:"결제 구분이 올바르지 않습니다."},{status:400});
      if(early&&!/^([01]\d|2[0-3]):[0-5]\d$/u.test(String(earlyTime))||late&&!/^([01]\d|2[0-3]):[0-5]\d$/u.test(String(lateTime)))return Response.json({error:"얼리체크인·레이트체크아웃 시간을 확인하세요."},{status:400});
      if(cardRef&&cardRef.replace(/\D/gu,"").length>=12)return Response.json({error:"원문 카드번호는 저장할 수 없습니다. PG 토큰 또는 마스킹된 참조만 입력하세요."},{status:400});
      const after={bookerName,bookerPhone:(body.bookerPhone||"").trim().slice(0,40)||null,bookerEmail:(body.bookerEmail||"").trim().slice(0,254)||null,guestFirstName,guestLastName,guestPhone:(body.guestPhone||"").trim().slice(0,40)||null,guestEmail:(body.guestEmail||"").trim().slice(0,254)||null,adults,children,channelProductName:(body.channelProductName||"").trim().slice(0,160)||null,paymentType,guestRequest:(body.guestRequest||"").trim().slice(0,2000),guestRequestResponse:(body.guestRequestResponse||"").trim().slice(0,2000),managerMemo:(body.managerMemo||"").trim().slice(0,2000),hotelMemo:(body.hotelMemo||"").trim().slice(0,2000),reservationChecked:checked,earlyCheckin:early,earlyCheckinTime:earlyTime,lateCheckout:late,lateCheckoutTime:lateTime,cardInfoRef:cardRef,serviceFeeIncluded:asFlag(body.serviceFeeIncluded),version:expectedVersion+1};
      await db.batch([
        db.prepare("UPDATE guests SET first_name=?,last_name=?,email=?,phone=? WHERE id=? AND property_id=pms_current_property_id()").bind(guestFirstName,guestLastName,after.guestEmail,after.guestPhone,String(reservation.guest_id)),
        db.prepare("UPDATE reservations SET booker_name=?,booker_phone=?,booker_email=?,channel_product_name=?,payment_type=?,guest_request=?,guest_request_response=?,manager_memo=?,hotel_memo=?,reservation_checked=?,early_checkin=?,early_checkin_time=?,late_checkout=?,late_checkout_time=?,card_info_ref=?,service_fee_included=?,adults=?,children=?,version=version+1,updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND version=?").bind(bookerName,after.bookerPhone,after.bookerEmail,after.channelProductName,paymentType,after.guestRequest,after.guestRequestResponse,after.managerMemo,after.hotelMemo,checked,early,earlyTime,late,lateTime,cardRef,after.serviceFeeIncluded,adults,children,now,body.reservationId,expectedVersion),
        db.prepare("INSERT INTO reservation_mutations VALUES (?,pms_current_property_id(),?,?, 'DETAIL_EDIT',?,?)").bind(crypto.randomUUID(),body.reservationId,expectedVersion,actor,now),
        db.prepare("INSERT INTO audit_logs VALUES (?,pms_current_property_id(),?,'UPDATE_RESERVATION_DETAIL','reservation',?,?,?,?)").bind(crypto.randomUUID(),actor,body.reservationId,jsonb({reservation,guest}),jsonb(after),now),
        db.prepare("INSERT INTO outbox_events VALUES (?,pms_current_property_id(),'reservation.updated','reservation',?,?,'PENDING',0,?,NULL)").bind(crypto.randomUUID(),body.reservationId,jsonb({reservationId:body.reservationId,version:expectedVersion+1,scope:"DETAIL"}),now),
        mutationReceipt(),
      ]);
    } else if (body.action === "link_reservation" && reservation) {
      const target=await db.prepare("SELECT id,confirmation_no FROM reservations WHERE property_id=pms_current_property_id() AND upper(confirmation_no)=upper(?) LIMIT 1").bind((body.linkedConfirmationNo||"").trim()).first<{id:string;confirmation_no:string}>();
      const relationType=String(body.relationType||"COMPANION"),notes=(body.notes||"").trim().slice(0,500);
      if(!target)return Response.json({error:"연계할 예약번호를 찾지 못했습니다."},{status:404});
      if(target.id===body.reservationId)return Response.json({error:"같은 예약을 서로 연계할 수 없습니다."},{status:400});
      if(!["COMPANION","CONSECUTIVE","GROUP"].includes(relationType))return Response.json({error:"연계 유형을 확인하세요."},{status:400});
      const [sourceId,linkedId]=[body.reservationId,target.id].sort();
      ...
      const requestedRatePlan=(body.ratePlan||String(reservation.rate_plan)).trim().toUpperCase(),ratePlan=await db.prepare("SELECT id,code FROM rate_plans WHERE property_id=pms_current_property_id() AND code=? AND active").bind(requestedRatePlan).first<{id:string;code:string}>();if(!ratePlan)return Response.json({error:"활성 요금제를 선택하세요."},{status:400});
      ...
      const statements:D1PreparedStatement[]=[
        ...
        // Changing the product must move both the human code and the FK. The
        // reservation snapshot trigger then freezes the newly selected product.
        db.prepare("UPDATE reservations SET room_type_id=?, room_id=?, arrival_date=?, departure_date=?, adults=?, children=?, rate_plan=?, rate_plan_id=?, nightly_rate=?, eta=?, notes=?, version=version+1, updated_at=? WHERE id=? AND property_id=pms_current_property_id() AND status='DUE_IN' AND version=?").bind(body.roomTypeId,retainedRoom,body.arrivalDate,body.departureDate,Math.max(1,Number(body.adults)||1),Math.max(0,Number(body.children)||0),requestedRatePlan,ratePlan.id,Number(body.nightlyRate)||Number(type.base_rate),body.eta||null,body.notes||"",now,body.reservationId,expectedVersion),
      ];
      ...
      const reservationId=crypto.randomUUID(),guestId=crypto.randomUUID(),confirmation=`SEL-${String(entry.arrival_date).replaceAll("-","").slice(2)}-${Math.floor(1000+Math.random()*9000)}`,stayDates=datesBetween(String(entry.arrival_date),String(entry.departure_date)); const statements:D1PreparedStatement[]=[
        ...
        db.prepare("INSERT INTO reservations(id,confirmation_no,property_id,guest_id,room_type_id,room_id,arrival_date,departure_date,status,adults,children,source,rate_plan,nightly_rate,eta,notes,version,created_at,updated_at,booker_name,channel_product_name,payment_type) VALUES (?, ?, pms_current_property_id(), ?, ?, NULL, ?, ?, 'DUE_IN', 1, 0, 'Group', ?, ?, NULL, ?, 1, ?, ?, ?, ?, 'DIRECT_BILL')").bind(reservationId,confirmation,guestId,String(entry.room_type_id),String(entry.arrival_date),String(entry.departure_date),ratePlan.code,Number(entry.rate),`Block ${entry.block_code} · Rooming list`,now,now,`${entry.first_name} ${entry.last_name}`,String(ratePlan.code)),
        db.prepare("INSERT INTO folio_windows VALUES (?, pms_current_property_id(), ?, 1, 'Guest Folio', 'GUEST', NULL, 'OPEN', ?, ?, NULL)").bind(`fw-${reservationId}`,reservationId,now,actor),
      ];
      ...
      const provider=body.provider?.trim().toUpperCase(),externalPropertyId=body.externalPropertyId?.trim();if(!provider||!externalPropertyId)return Response.json({error:"채널과 외부 호텔 ID를 입력하세요."},{status:400});const connectionId=crypto.randomUUID();await db.batch([db.prepare("INSERT INTO channel_connections VALUES (?, pms_current_property_id(), ?, ?, ?, 'SANDBOX', 'ACTIVE', NULL, ?, ?, ?)").bind(connectionId,provider,externalPropertyId,body.name?.trim()||`${provider} Sandbox`,now,now,actor),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_CHANNEL_CONNECTION', 'channel_connection', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,connectionId,jsonb({provider,externalPropertyId,environment:"SANDBOX"}),now),mutationReceipt()]);
    } else if (body.action === "create_channel_mapping") {
      const connection=await db.prepare("SELECT id FROM channel_connections WHERE id=? AND property_id=pms_current_property_id() AND status='ACTIVE'").bind(body.connectionId).first(),roomType=await db.prepare("SELECT id FROM room_types WHERE id=? AND property_id=pms_current_property_id()").bind(body.roomTypeId).first(),requestedRatePlan=(body.ratePlan||"OTA").trim().toUpperCase(),ratePlan=await db.prepare("SELECT code FROM rate_plans WHERE property_id=pms_current_property_id() AND code=? AND active").bind(requestedRatePlan).first();if(!connection||!roomType||!ratePlan||!body.externalRoomTypeId?.trim()||!body.externalRatePlanId?.trim())return Response.json({error:"활성 연결, 객실 타입, 내부 요금제, 외부 room/rate ID를 입력하세요."},{status:400});const mappingId=crypto.randomUUID();await db.batch([db.prepare("INSERT INTO channel_mappings VALUES (?, pms_current_property_id(), ?, ?, ?, ?, ?, true, ?, ?)").bind(mappingId,body.connectionId,body.roomTypeId,body.externalRoomTypeId.trim(),requestedRatePlan,body.externalRatePlanId.trim(),now,now),db.prepare("INSERT INTO audit_logs VALUES (?, pms_current_property_id(), ?, 'CREATE_CHANNEL_MAPPING', 'channel_mapping', ?, NULL, ?, ?)").bind(crypto.randomUUID(),actor,mappingId,jsonb({connectionId:body.connectionId,roomTypeId:body.roomTypeId,ratePlan:requestedRatePlan,externalRoomTypeId:body.externalRoomTypeId,externalRatePlanId:body.externalRatePlanId}),now),mutationReceipt()]);
    } else if (body.action === "queue_ari_delta") {
      const mapping=await db.prepare("SELECT m.*,c.provider FROM channel_mappings m JOIN channel_connections c ON c.id=m.connection_id JOIN property_channel_settings pcs ON pcs.connection_id=c.id AND pcs.property_id=c.property_id AND pcs.active WHERE m.id=? AND m.property_id=pms_current_property_id() AND c.property_id=pms_current_property_id() AND m.active AND c.status='ACTIVE'").bind(body.mappingId).first<Record<string,unknown>>(),dates=datesBetween(body.startDate,(()=>{const end=new Date(`${body.endDate}T00:00:00Z`);end.setUTCDate(end.getUTCDate()+1);return end.toISOString().slice(0,10)})());
      ...
      const update=await db.prepare("SELECT a.*,c.provider FROM ari_updates a JOIN channel_connections c ON c.id=a.connection_id JOIN property_channel_settings pcs ON pcs.connection_id=c.id AND pcs.property_id=c.property_id AND pcs.active WHERE a.id=? AND a.property_id=pms_current_property_id() AND c.property_id=pms_current_property_id() AND c.status='ACTIVE' AND a.status IN ('PENDING','FAILED')").bind(body.updateId).first<Record<string,unknown>>();if(!update)return Response.json({error:"전송 또는 재처리 가능한 ARI 업데이트가 없습니다."},{status:409});const failed=body.outcome==="FAIL",attempt=Number(update.attempts)+1;await db.batch([db.prepare("UPDATE ari_updates SET status=?,attempts=?,sent_at=?,last_error=? WHERE id=? AND property_id=pms_current_property_id()").bind(failed?"FAILED":"SENT",attempt,failed?null:now,failed?"SANDBOX_TIMEOUT":null,update.id),db.prepare("UPDATE channel_connections SET last_sync_at=CASE WHEN ?=1 THEN last_sync_at ELSE ? END,updated_at=? WHERE id=? AND property_id=pms_current_property_id()").bind(failed?1:0,now,now,update.connection_id),db.prepare("INSERT INTO integration_delivery_attempts VALUES (?, pms_current_property_id(), 'OUTBOUND', ?, 'ari_update', ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(),update.provider,update.id,attempt,failed?"FAILED":"ACKED",failed?504:200,failed?"TIMEOUT":null,failed?"Sandbox timeout":null,update.payload_json,now,actor),mutationReceipt()]);
