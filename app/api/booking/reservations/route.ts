...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await createWebReservation(await jsonBody(request) as ReservationInput, idempotencyKey);
    ...
    if (message.includes("booking_request_idempotency_uq") || message.includes("booking_requests_property_idempotency")) {
      const existing = await findWebReservationByIdempotency(idempotencyKey);
