...
function BoardReservationCard({span,canWrite,onOpen,onDrag,onChoose,onUnassign}:{span:BoardSpan;canWrite:boolean;onOpen:(row:FrontdeskReservation)=>void;onDrag:(item:DragItem|null)=>void;onChoose:(item:DragItem)=>void;onUnassign:(row:FrontdeskReservation)=>void}){
  const row=span.reservation;
  return <article className={`room-board-stay ${row.status.toLowerCase()}`}>
    <button type="button" className="room-board-stay-main" draggable={canWrite} aria-grabbed="false" onDragStart={event=>{event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/plain",row.id);onDrag({reservation:row,span});}} onDragEnd={()=>onDrag(null)} onClick={()=>onOpen(row)}>
      <b>{row.first_name} {row.last_name}</b><span>{row.confirmation_no} · {statusLabel[row.status]||row.status}</span>
    </button>
    {canWrite&&<div className="room-board-stay-actions"><button type="button" aria-label={`${row.first_name} ${row.last_name} 객실 선택`} onClick={()=>onChoose({reservation:row,span})}>객실 선택</button>{row.status!=="IN_HOUSE"&&<button type="button" aria-label={`${row.first_name} ${row.last_name} 배정 해제`} onClick={()=>onUnassign(row)}>해제</button>}</div>}
  </article>;
}
...
export default function FrontdeskRoomBoard({businessDate,canWrite,onOpen,onCreate}:{businessDate:string;canWrite:boolean;onOpen:(row:FrontdeskReservation)=>void;onCreate:(prefill:CreatePrefill)=>void}){
  ...
  const [dragged,setDragged]=useState<DragItem|null>(null),[dialog,setDialog]=useState<AssignmentDialog|null>(null),[roomPicker,setRoomPicker]=useState<DragItem|null>(null);
  ...
  const unassign=async(row:FrontdeskReservation)=>{if(!window.confirm(`${row.first_name} ${row.last_name} 예약의 물리 객실 배정을 해제할까요?\n객실 타입 재고는 유지됩니다.`))return;await act("unassign_reservation_room",{reservationId:row.id,expectedVersion:String(row.version)});};
  return <section className="panel full frontdesk-room-board" aria-busy={query.isFetching}>
    ...
    {data&&<>
      <aside className="room-board-unassigned" aria-label="미배정 예약"><header><div><b>미배정 예약</b><span>{data.unassigned.length}건 · 객실로 끌어 배정</span></div></header><div>{data.unassigned.map(row=><article key={row.id} draggable={canWrite} aria-grabbed="false" onDragStart={event=>{event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/plain",row.id);setDragged({reservation:row,span:null});}} onDragEnd={()=>setDragged(null)}><button type="button" onClick={()=>onOpen(row)}><b>{row.first_name} {row.last_name}</b><span>{row.arrival_date.slice(5)} → {row.departure_date.slice(5)} · {row.room_type_code}</span><small>{row.confirmation_no}</small></button>{canWrite&&<button type="button" className="secondary" onClick={()=>setRoomPicker({reservation:row,span:null})}>객실 선택</button>}</article>)}{!data.unassigned.length&&<p>조회 기간에 미배정 예약이 없습니다.</p>}</div></aside>
    ...
    {dialog&&<AssignmentDecision dialog={dialog} busy={busy} close={()=>setDialog(null)} submit={async(mode)=>{const row=dialog.item.reservation,payload={reservationId:row.id,roomId:dialog.room.id,expectedVersion:String(row.version),warningOverride:"true"};const ok=await act(mode==="FROM_DATE"?"move_reservation_room":"assign_reservation_room",mode==="FROM_DATE"?{...payload,moveDate:dialog.dropDate,reason:"ROOM_BOARD"}:payload);if(ok)setDialog(null);}}/>}
  </section>;
}
