...
### ADR-029 · 검색 exact/fuzzy 실행계획과 용량 시험 분리

exact BTREE와 trigram GIN 조건을 하나의 OR 실행계획에 넣으면 PostgreSQL이 100,000실 공통 token에도 similarity 후보를 계산해 선형 지연이 발생합니다. 검색은 exact token 후보를 먼저 materialize하고 한 건이라도 있으면 fuzzy 경로를 억제합니다. 한글·자모가 없는 Latin·숫자 입력은 compact 값과 같은 초성 GIN을 다시 실행하지 않습니다.

검증도 목적별로 분리합니다. PR의 2,000실 시험은 실제 행 trigger가 검색 문서·토큰을 유지하는 비용과 읽기 정확성을 함께 검증합니다. 주간 100,000실 시험은 같은 production table, normalized document/term shape, BTREE/GIN index에 set-based fixture를 넣어 read capacity와 동시 p95를 검증합니다. 두 시험 모두 loopback DB와 잔여 데이터 0건을 강제합니다.
