...
- 카드정보: 원문 PAN은 받지 않는다. 최대 160자의 PG token 또는 마스킹 참조만 허용하며 공백·하이픈 등 구분자를 제외한 숫자가 12자리 이상이면 DB CHECK와 command에서 모두 거부한다.
...
- `/frontdesk/imports`: Excel에서 UTF-8 CSV로 저장할 수 있는 예약 양식을 내려받고 최대 2,000행을 dry-run한다. 필수 헤더, 날짜 범위, 객실타입, 활성 Rate Plan, 확인번호, 고객 참조를 검증하고 오류가 0건인 job만 원자 commit한다. 모든 POST는 기본적으로 Supabase AAL2 MFA를 요구하며 commit/rollback은 job kind가 `RESERVATIONS`인지 다시 확인한다. 고객 외부 ID가 아직 없으면 행의 고객 이름·이메일·전화로 guest와 mapping을 같은 commit에 생성하고, flat nightly rate를 모든 숙박일의 immutable `reservation_rate_nights`에 보존한다. 파일 SHA-256과 `(property,kind,content_hash,mode)` unique 계약으로 같은 파일 commit 재시도는 기존 job receipt를 반환하며, 이관 후 바뀌지 않은 예약과 해당 job 소유 야간 원장만 안전 rollback한다.
