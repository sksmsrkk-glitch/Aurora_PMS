...
- [ ] Vercel Production에 `SUPABASE_URL`, `SUPABASE_SECRET_KEY`, `DATABASE_URL`, `PMS_RATE_LIMIT_SECRET`, `SEARCH_CURSOR_SECRET` 존재
...
- [ ] `202607190016_multihotel_saas_control_plane`부터 `202607230035_search_term_candidate_performance`까지 순서대로 적용하고 자동 tenant policy 계약 확인
...
- [x] 검색 2,000실 PR smoke와 주간 100,000실·8동시 capacity gate
- [ ] 전체 PMS 업무별 대량 데이터·동시 사용자 목표 SLA의 운영 compute 재확정
...
- 공개 부킹 엔진은 결제사 키가 없어 현장 결제만 지원합니다. 샘플 호텔은 3~8층 160실과 8개 표준 객실 타입을 게시하며, 이후 판매 대상은 코드가 아니라 `room_type_website.published`로 관리합니다.
...
| Commit | 작업 |
| --- | --- |
| 2026-07-23 sample room catalog | QA 생성 객실 타입·9층 잔재 제거, 예약/요금 참조 보존, 8개 표준 타입과 3~8층 160실·홈페이지·Rate Plan projection 원자 재구축 |
...
| 2026-07-23 tenant search platform | migration 0030→0035 순서로 tenant 검색 문서/토큰, 비식별 품질 집계, keyset, 로마자 별칭, token-first BTREE/GIN 후보 경로; RLS/trigger 행동 테스트, 2,000실 trigger·100,000실/8동시 benchmark, 인증 모바일 UI QA |
...
| 2026-07-22 finance/rate/snapshot audit | RESTORE 원금 동치, 다단계 Rate Plan 상속, 예약 snapshot 불변 trigger, 배정 셀 클릭 차단, 플랫폼·임포트 오류 ID 세정 |
