...
Talos PMS는 예약, 장기 객실 재고·요금, 프런트 데스크, 하우스키핑, 그룹 블록, 폴리오, 매출채권, 채널 계약·정산, 호텔 회계·손익, 캐셔, 야간 감사와 운영 리포트를 하나의 운영 원장으로 연결하는 차세대 호텔 PMS(Property Management System)입니다.
...
| 전체 업무 QA | 격리 환경 `qa:workflow` 24개 핵심 checkpoint + 중단 캐셔 조건부 복구, booking/CMS·공개 SEO E2E gate |
...
| 연회 운영 | 완료 | 연회장 master, 월 캘린더, 행사·시간·인원·요금·상태, 병렬 시간 충돌 차단 |
...
| 리포트·Excel | 완료 | 16종, 리드타임·예약곡선·YoY·채널입금/복구·후불·검색품질, 키워드·복합 필터, 마스킹, CSV/XLSX, export audit |
| 예약 일괄 등록 | 완료 | Excel용 CSV 양식, dry-run 행 검증, 내장 고객, 원자 commit/replay, 변경 감지 rollback |
| 호텔·웹 회원 | 완료 | 회원 코드·ID·연락처·회사·등급·활성, scrypt 비밀번호, 지원 PII 마스킹 |
| 야간 감사 | 완료 | blocker, 객실료 전기, cutoff, 영업일 전환 |
| UI 시스템 | 완료 | Talos Flow UI, Toss Product Sans 실제 CDN 로드, 반응형·접근성 상태 |
...
| 홈페이지 관리 CMS | 완료 | 실시간 반응형 비주얼 에디터, 히어로·메뉴 커스터마이징, 호텔/객실 이미지, 객실 소개·편의시설, 타입 생성·게시, Supabase Storage, 홈페이지 전용 일자별 판매 중지 |
...
- [HotelStory 1:1 벤치마크·A–N 이식 추적표](hotelstory-parity.md)
- [멀티호텔 SaaS·Control Plane·지원·이관·worker·백업](multihotel-saas.md)
...
- [검색 무결성·100,000실 용량·UI QA](search-integrity.md)
