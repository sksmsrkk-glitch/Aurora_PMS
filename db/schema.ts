/** Drizzle schema mirror used for local tooling and relational documentation. */
...
export const channelContracts = sqliteTable("channel_contracts", {
  id:text("id").primaryKey(), propertyId:text("property_id").notNull(), connectionId:text("connection_id").notNull(), contractType:text("contract_type").notNull(), commissionPercent:real("commission_percent").notNull().default(0), settlementCycle:text("settlement_cycle").notNull().default("PER_STAY"), paymentTermsDays:integer("payment_terms_days").notNull().default(30), currency:text("currency").notNull().default("KRW"), validFrom:text("valid_from").notNull(), validTo:text("valid_to"), status:text("status").notNull().default("ACTIVE"), version:integer("version").notNull().default(1), createdAt:text("created_at").notNull(), createdBy:text("created_by").notNull(), updatedAt:text("updated_at").notNull(), updatedBy:text("updated_by").notNull(),
},(t)=>[uniqueIndex("channel_contract_connection_uq").on(t.connectionId),index("channel_contract_property_status_idx").on(t.propertyId,t.status,t.validFrom)]);
...
export const accountingAccounts = sqliteTable("accounting_accounts", {
  id:text("id").primaryKey(), propertyId:text("property_id").notNull(), code:text("code").notNull(), name:text("name").notNull(), accountType:text("account_type").notNull(), category:text("category").notNull(), department:text("department"), externalCode:text("external_code"), active:integer("active",{mode:"boolean"}).notNull().default(true), version:integer("version").notNull().default(1), createdAt:text("created_at").notNull(), updatedAt:text("updated_at").notNull(),
},(t)=>[uniqueIndex("accounting_account_code_uq").on(t.propertyId,t.code),index("accounting_account_type_idx").on(t.propertyId,t.accountType,t.active)]);
...
export const websiteMedia = sqliteTable("website_media", {
  id:text("id").primaryKey(),propertyId:text("property_id").notNull(),scope:text("scope").notNull(),roomTypeId:text("room_type_id"),role:text("role").notNull(),objectPath:text("object_path").notNull(),publicUrl:text("public_url").notNull(),altText:text("alt_text").notNull(),sortOrder:integer("sort_order").notNull().default(0),active:integer("active",{mode:"boolean"}).notNull().default(true),createdAt:text("created_at").notNull(),createdBy:text("created_by").notNull(),
},(t)=>[index("website_media_public_idx").on(t.propertyId,t.scope,t.roomTypeId,t.active,t.sortOrder)]);
